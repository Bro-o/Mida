...
echo "Downloading trace YCSB RocksDB SSD Traces Part 00" 1>&2
cookies=$(mktemp)
cat >> $cookies << 'EOF'
...
downloadFile() {
  ...
  if [ $error -eq 0 ]; then
    ...
    downloadLink="http://iotta.snia.org/downloaderinfos/new?trace_id=28571&type=script&sType=&came_from=iotta.snia.org"
    if $delete; then
